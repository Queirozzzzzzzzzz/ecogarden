...
class ManageEnvironmentGoalFragment : Fragment() {
    ...
    private fun setElements() {
        ...
        optionIds.forEach { optionId ->
            ...
            checkbox.setImageResource(if (TemporaryFormData.goals?.contains(optionId.toString()) == true) R.drawable.checkbox_checked else R.drawable.checkbox_unchecked)
        }

    }

    private fun toggleCheckbox(goal: String, checkbox: ImageView) {
        val newGoals = TemporaryFormData.goals?.toMutableSet() ?: mutableSetOf()
        if(goal in newGoals) {
        ...
        TemporaryFormData.goals = newGoals.toList()
    }
}
